...
Jamieson and Mewhort (2009) created a model that reproduced the Hick-Hyman law on the SRT solely through a local-context dependent mechanism.  We include a description of their model to illustrate a possible mechanism by which uncertainty slows responses.  Their model encoded each memory trace as a concatenation of three vectors: the immediately preceding response, the current stimulus and the current response.  Their model in essence stored instances that associated prior contexts with their respective responses.  Each probe or stimulus was the concatenation of two vectors: the immediately preceding response and the current stimulus.  The model's task was to select the most appropriate current response for the probe vector from the stored memory traces.  A Similarity score is calculated between the probe's vector and each concatenated vector of the encoded memory traces.  The model selects the best matching memory trace by iteratively exaggerating the differences between memory traces' similarity scores until a clear winner can be chosen.  To produce the Hick-Hyman law, Jamieson and Mewhort made their model undergo practice trials in which the model accumulated memory traces from each of the 24 uncertainty conditions in Hyman's 1953 study.  After their practice trials, when the response times of the model were measured again, IKSI had a strong positive correlation with uncertainty, replicating the results of Hyman's 1953 study.  

Our model's performance is consistent with that of the computational models of Jamieson and Mewhort.  The same factors that increase retrieval time in our model increase retrieval time in their model.  Jamieson and Mewhort model produces longer retrieval times when there is a large amount of memory trace vectors that are equally similar to the probe.  In other words, when their model attempts to retrieve a response in a context that has many possible follow-up responses stored in memory, it slows down.  Jamieson and Mewhort's model produces longer retrieval times because the model needs more iterations to exaggerate differences between all the memory traces' similarity scores. Our model slows down in similar circumstances; when the n-1 letter does not have one clear follow-up letter, (high n-1 uncertainty), the IKSIs increase.    

While our model demonstrates that IKSIs increase with uncertainty, it does not necessitate a causal relationship.  Repeatability may be an important determinant of IKSIs in continuous typing.  In addition to varying uncertainty, Jamieson and Mewhort tested their model's performance under varying redundancy conditions.  Their model produced longer response times when redundancy was decreased.  Kornblum 1969, demonstrated that the repeatability of SRTs can be manipulated while keeping uncertainty constant.  He was able to create a choice reaction task in which equi-uncertainty stimuli  would ellicit varying reaction times by varying the repeatability.   Future research should examine the relative contributions of repeatability and uncertainty to IKSI variance.  
